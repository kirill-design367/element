...
/**
 * ЭТО ЗАГРУЗЧИК НАСТРОЕК, А НЕ ФАЙЛ НАСТРОЕК. ДОСТУПОВ ЗДЕСЬ НЕТ НИ ОДНОГО.
 *
 * Файл назывался `config.php`, и имя сбивало с толку: лежит внутри корня
 * сайта, называется как файл настроек — и читается как «пароли в открытом
 * доступе». На этом уже потеряли одно разбирательство, поэтому имя и стало
 * `config-load.php`: оно говорит, что файл ЗАГРУЖАЕТ настройки, а не хранит.
 * Внутри только код, который ищет настоящий файл и подключает его по
 * абсолютному пути.
 *
 * Сам файл настроек — с паролем от базы, токеном GitHub и ключом выгрузки —
 * лежит ВНЕ корня сайта: по FTP это `/cms-config.php`, рядом с папкой `www`.
 * Оттуда его не отдать по HTTP никакой конфигурацией, и выкладка с
 * `--delete` до него не дотягивается.
 *
 * Что доступов в `public/` нет, проверяет сборка —
 * `scripts/no-secrets-in-public.mjs`.
 */
...
function cms_config_fail(?string $reason): never
{
    ...
    if ($reason !== null) {
        ...
        /* Образец лежит в репозитории, в `db/cms-config.sample.php`, и на
           сервер не выкладывается — вместе с установщиком. Ссылаться отсюда
           на `/admin/cms-config.sample.php` нельзя: этого адреса больше нет. */
        echo '<p>Нет файла настроек. Возьмите образец <code>db/cms-config.sample.php</code> '
            . 'из репозитория, впишите доступы и положите копию под именем '
            . '<code>cms-config.php</code> по FTP в корень аккаунта — рядом с папкой '
            . '<code>www</code>, а не внутрь сайта.</p>';
