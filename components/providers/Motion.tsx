...
import { DUR, EASE, EASE_REVEAL, PARALLAX_FROM_PERCENT, REVEAL, prefersReducedMotion } from '@/lib/motion';
    ...
    const start = async () => {
      ...
      const onAnchor = (e: MouseEvent) => {
        ...
        scroll.scrollTo(target as HTMLElement, {
          /* Своего отступа здесь нет. Стоял offset: -96, и он ложился ПОВЕРХ
             scroll-margin-top цели: секции объявляют scroll-mt-28 (112 px),
             и Lenis этой версии его читает. Замер: пункт «Расчёт» приводил
             секцию на 208 px от кромки против 112 в режиме покоя, где
             прокрутку ведёт браузер. Отступ теперь один и объявлен там же,
             где у остальных якорей, — в разметке цели. */
          duration: 1.1,
      ...
      const ctx = gsap.context(() => {
        ...
        parallax.forEach((el) => {
          /* Триггер — рамка вокруг кадра, а не сам кадр. Отсчёт начинался от
             элемента, который этот же триггер и двигает: у кадра первого
             экрана собственный сдвиг -24 px плюс начало параллакса, и
             «top top» разрешался в отрицательную прокрутку. На нулевом
             скролле прогресс выходил не 0, а 0,045 — кадр вставал на 1,72 px
             мимо своего статического положения, и это ровно тот остаток
             скачка, который не убирался ничем другим.

             Начинаем с родителя и поднимаемся до первого предка с высотой:
             у кадра это <picture>, инлайновый элемент нулевой высоты, а над
             ним .photo-box во всю секцию. */
          let box: HTMLElement = el.parentElement ?? el;
          ...
          /* Отрезок считается в пикселях от живой высоты кадра, а не через
             yPercent, и это не придирка.

             Кадр рождается сдвинутым на начало отрезка — инлайновый
             translateY(-2%) в разметке (PARALLAX_FROM_PERCENT в
             lib/motion.ts). Дальше должно совпасть знак в знак, иначе первый
             же кадр гасапа даст скачок. Два замера по дороге:

             1. Без y: 0 gsap не заменяет transform, а РАЗБИРАЕТ его:
                инлайновый сдвиг он читает как базовый y в пикселях и
                добавляет yPercent сверху. Статическое -42,96 превращалось
                в -59,44 — скачок остался прежним по величине.
             2. С y: 0 и yPercent gsap писал -17,24 при том, что CSS даёт
                -18,96 от той же высоты 948 px. Основание процента у него
                своё, и совпасть с CSS оно не обязано: оставалось 1,72 px.

             Функция от offsetHeight снимает вопрос совсем: 2% считаются от
             того же числа, от которого их считает CSS. invalidateOnRefresh
             пересчитывает их при смене высоты экрана — ради чего проценты и
             брались. */
          const shift = () => (el.offsetHeight * PARALLAX_FROM_PERCENT) / 100;
          /* Приведение состояния перед постройкой. gsap не читает CSS-процент
             так же, как браузер: инлайновый translateY(-2%) он разобрал в
             -17,24 px там, где CSS рисует -18,96 (высота кадра 948, замер в
             тот же кадр). Скраб начинал ход от своего числа, и оставался
             скачок в 1,72 px. gsap.set переписывает состояние ровно тем же
             сдвигом в пикселях — на экране не меняется ничего, а дальше
             начало отрезка совпадает с ним знак в знак. */
          gsap.set(el, { y: shift(), yPercent: 0 });
          gsap.fromTo(
            el,
            { y: shift, yPercent: 0 },
            {
