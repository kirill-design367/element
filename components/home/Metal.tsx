...
import { groupsOf, materialsOf } from '@/lib/catalog';
import { plural, typo } from '@/lib/format';

/**
 * Металлопрокат.
 *
 * Карточка была анонсом: номенклатуры не было, и в тексте не было ни одной
 * цифры. Теперь прайс пришёл, металл заведён шестой категорией каталога, и
 * карточка говорит то, что есть: сколько позиций, какие группы, в чём цена.
 * Число позиций и названия групп берутся из lib/catalog.ts — руками здесь не
 * набрано ни одно из них.
 *
 * Марок стали, ГОСТов, сроков и минимальной партии в прайсе нет, поэтому их
 * нет и в тексте.
 *
 * Кнопок две и стоят они парой, как на первом экране: главная ведёт в
 * заявку, вторичная открывает каталог с уже выставленным фильтром по
 * металлу — тем же адресом, что и карточка категории в ленте.
 *
...
export function Metal() {
  const count = materialsOf('metall').length;
  const groups = groupsOf('metall')
      ...
      <div className="order-2 lg:order-1 lg:col-span-6 lg:flex lg:flex-col lg:justify-center">
        <div className="narrow narrow-left lg:max-w-none lg:pr-10">
          ...
          <p className="mt-4 max-w-[46ch] text-t2 leading-relaxed text-ink-2">
            {typo(
              `Помимо инертных материалов возим металлопрокат. В каталоге ${count} ${plural(count, 'позиция', 'позиции', 'позиций')}: ${groups}. Цены за тонну. Марку стали, наличие и срок называем по заявке — в прайсе их нет.`,
            )}
          </p>
          {/* Пара кнопок в том же порядке, что на первом экране: главная
              первой, вторичная за ней. На телефоне встают в столбец. */}
          <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center">
            <ButtonLink href="#zayavka" className="w-full sm:w-auto">
              Запросить прайс
            </ButtonLink>
            <ButtonLink
              href="/catalog/?category=metall"
              variant="secondary"
