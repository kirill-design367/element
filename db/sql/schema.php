<?php
/* SQL ЛЕЖИТ В PHP-ФАЙЛЕ, А НЕ В .sql, И ЭТО НЕ ПРИЧУДА.
 *
 * Спереди на хостинге стоит nginx, и статику он отдаёт САМ, по расширению,
 * мимо Apache — то есть мимо .htaccess. Файл с расширением .sql в корне
 * сайта был бы доступен любому, и запрет из .htaccess до него мог бы и не
 * дойти. Секретов внутри нет — это схема и первоначальное наполнение, — но
 * состав базы посторонним знать незачем.
 *
 * У PHP-файла такой беды нет: его исполняет Apache, и наружу он отдаёт
 * пустоту. Прямое обращение сюда не покажет ничего.
 */
return <<<'SQL'
