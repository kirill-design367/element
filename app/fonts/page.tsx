...
export const metadata: Metadata = {
  ...
  description:
    'Три шрифтовые пары для сайта на реальных кусках лендинга и каталога. Кириллица и табличные цифры проверены по таблице cmap в бинарнике.',
};
...
const plex = localFont({
  src: [{ path: '../../assets/fonts/IBMPlexSans.woff2', weight: '400 700', style: 'normal' }],
  display: 'swap',
  preload: false,
});
...
const PAIRS: Pair[] = [
  ...
  },
  {
    id: 'tektur-onest',
    name: 'Tektur + Onest',
    role: 'Заголовки и числа — Tektur, с живой осью ширины. Интерфейс и текст — Onest.',
    heading: tektur.style.fontFamily,
    body: onest.style.fontFamily,
    headingStretch: '82%',
    license: 'Обе — SIL Open Font License 1.1',
    price: 'Бесплатно, в том числе в коммерческом проекте',
    buyLabel: 'Tektur на Google Fonts',
    buyHref: 'https://fonts.google.com/specimen/Tektur',
    checks: [byFile('Tektur.ttf'), byFile('Onest.ttf')],
    why: 'Прямая ставка на характер отрасли. Tektur построен на прямоугольнике со срезанными углами — так рисуют шильдики на технике и трафареты на бортах самосвалов. У него единственного из проверенных есть перечёркнутый ноль (фича zero), а это ровно то, чем инженерная документация отличается от рекламной: ноль нельзя спутать с буквой «о» в марке прочности М1000. Живая ось ширины 75–100 решает главную беду русских заголовков. В колонке 780 пикселей заголовок первого экрана занимает две строки — столько же, сколько у нынешней пары; но на телефоне, в колонке 282 пикселя, поджатый до 82 процентов Tektur встаёт в две строки против трёх у Geologica и трёх у самого себя при обычной ширине. На первом экране это выигранный экран прокрутки. Onest рядом работает молча: широкая апертура и крупный рост строчных держат мелкий интерфейсный текст читаемым на телефоне.',
  ...
  },
  {
    id: 'cofo-sans',
    name: 'CoFo Sans',
    role: 'Одна суперсемья на обе роли: заголовки, числа, интерфейс и текст.',
    heading: plex.style.fontFamily,
    body: plex.style.fontFamily,
    license: 'Коммерческая лицензия Contrast Foundry (десктоп, веб, приложение — раздельно)',
    price: 'Цену смотреть на сайте словолитни: из сборочной среды он недоступен',
    buyLabel: 'CoFo Sans в Contrast Foundry',
    buyHref: 'https://contrastfoundry.com/ru/typeface/cofo-sans',
    substitute: 'IBM Plex Sans',
    checks: [byFile('IBMPlexSans.ttf')],
    ...
    weak: 'Платная, и это единственный вариант, который нельзя поставить сегодня. Хуже того — на этой странице она набрана не собой: сайт словолитни закрыт сетевой политикой сборочной среды, файла нет, и композиции ниже показаны подменой. Оценивать по ним можно пропорции и плотность, но не рисунок букв. Полноту кириллицы и табличные цифры CoFo Sans я по бинарнику не проверял — до покупки это надо сделать на пробном файле.',
  },
  {
    id: 'geologica-onest',
    name: 'Geologica + Onest',
    role: 'Сейчас на сайте. Заголовки и числа — Geologica, интерфейс — Onest.',
    heading: 'var(--font-display)',
    body: 'var(--font-text)',
    license: 'Обе — SIL Open Font License 1.1',
    price: 'Бесплатно',
    buyLabel: 'Geologica на Google Fonts',
    buyHref: 'https://fonts.google.com/specimen/Geologica',
    checks: [byFile('Geologica.ttf'), byFile('Onest.ttf')],
    baseline: true,
    why: 'Стоит на сайте сейчас и приведена здесь для честного сравнения: те же пять композиций, тот же кегль. Пара исправная — кириллица полная, табличные цифры на месте, файл лёгкий, контраст проходит с запасом. Претензий по технике нет.',
    weak: 'Ничего не говорит. Geologica — скандинавский гротеск норвежского бюро Monokrom, у него ровный нейтральный рисунок без единой запоминающейся детали, и в паре с таким же нейтральным Onest сайт получает голос диктора, а не поставщика. Заказчик сформулировал это точнее всего: не мешает и не помогает.',
  },
];

export default function FontsPage() {
  return (
    <div className={`${geologica.variable} ${onest.variable}`}>
      <div className="shell py-8 md:py-14">
        ...
        <p className="mt-3 max-w-[64ch] text-[16px] leading-relaxed text-ink-2 md:text-[17px]">
          На лендинге и в каталоге до вашего решения ничего не меняется: там стоит нынешняя пара.
          Кандидаты грузятся только этим маршрутом.
        </p>
  ...
  return (
    <section className="mt-9 rounded-card border border-line bg-surface p-5 shadow-card md:mt-12 md:p-7">
      ...
      <p className="mt-5 max-w-[70ch] text-[14px] leading-relaxed text-ink-2">
        ...
        выводится ниже — чтобы описание не разошлось с фактом. Решение за вами и пока не принято.
      </p>
    </section>
  );
}
      ...
      <header className="flex flex-col gap-4 border-b border-line pb-6 md:flex-row md:items-start md:justify-between md:gap-10">
        <div className="max-w-[52ch]">
          <p className="text-[12px] uppercase tracking-[.08em] text-ink-2">
            {pair.baseline ? 'Сейчас на сайте' : `Вариант ${index + 1}`}
          </p>
