...
const DIRECTIONS: Direction[] = [
  ...
  {
    title: 'Скол как разрыв',
    brief:
      'Слово в две строки — ЭЛЕ и МЕНТ, — скол лежит на стыке как место разлома. Просвет между строками задан долей высоты прописных, знак привязан к нему, а не поставлен рядом.',
    variants: [
      {
        id: 'razryv-shov',
        no: 9,
        name: 'Знак в шве',
        tagline: 'Просвет 0,5 высоты, знак ровно заполняет его, слева.',
        done: 'Слово разложено в две строки. Просвет между ними равен половине высоты прописных — 340 единиц, — и знак ровно заполняет его: его высота равна просвету, а не подобрана. Стоит по левому краю блока, на одной оси с началом обеих строк.',
        strong:
          'Компактнее всех однострочных: блок почти квадратный и в аватарку садится целиком, без компактной формы. Разлом читается сразу — слово разорвано, и в разрыве лежит скол.',
        weak: 'Знак вдвое меньше, чем в остальных направлениях, и в шве читается точкой-маркером, а не знаком: отдельно от слова он остаётся собой, но в лок-апе теряет вес до незаметного.',
      },
      {
        id: 'razryv-hvost',
        no: 10,
        name: 'Знак хвостом первой строки',
        tagline: 'Строки вплотную, знак в полный рост в конце первой.',
        done: 'Строки сдвинуты вплотную — просвет 0,18 высоты прописных, 122 единицы, — а знак поставлен в конце первой строки на её базовую линию, в полную высоту прописных. Первая строка короче второй, и место справа от неё уже пустовало.',
        strong:
          'Знак вернул себе полный рост и при этом стоит внутри композиции, а не рядом с ней. Блок читается как одно целое: разлом идёт по строке, а не между двумя объектами.',
        weak: 'Знак встал на место буквы в конце строки — и снова читается буквой, которой нет. Порядок чтения ломается: глаз идёт «ЭЛЕ», потом знак, потом «МЕНТ», и знак попадает в середину слова, а не в его начало или конец.',
      },
      {
        id: 'razryv-sdvig',
        no: 11,
        name: 'Строки разъехались',
        tagline: 'Вторая строка сдвинута на 0,42 высоты, знак в точке сдвига.',
        done: 'Вторая строка сдвинута вправо на 0,42 высоты прописных — 286 единиц, — блок будто сполз по плоскости разлома. Знак стоит в шве ровно между левыми краями разъехавшихся строк, то есть в самой точке сдвига.',
        strong:
          'Сдвиг делает разлом не декоративным, а физическим: видно, куда поехало. Из трёх вариантов разрыва этот единственный, где место знака выведено из композиции, а не выбрано.',
        weak: 'Сдвинутая строка ломает левую линию, и логотип нечем выровнять: в шапке, в подвале и на бланке ему потребуется своя ось. А на мелком размере сдвиг читается опечаткой набора, а не приёмом.',
      },
    ],
  },
];
