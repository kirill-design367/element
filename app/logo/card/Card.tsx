...
/* КАДР ИДЁТ ВО ВСЮ ШИРИНУ ЛИСТА, поля только чтобы он не упирался в обрез:
   6 мм против 14 у набора. Скругление то же, что у карточек показателей и
   выноски, — 6 мм: кадр остаётся частью одной системы форм, но перестаёт
   быть карточкой среди карточек и начинает держать верх листа сам.
   Полем набора его равнять нельзя: на 14 мм кадр читался утопленным. */
...
/* ЛОГОТИП ЛЕЖИТ ВНУТРИ КАДРА, в его нижнем левом углу. Отступы от нижней и
   левой кромки одинаковые — 11 мм; при скруглении 6 мм этого хватает, чтобы
   знак не подошёл к дуге угла (его угол отстоит от центра дуги на 7,07 мм
   при радиусе 6). Раньше знак стоял на стыке кадра и бумаги, но у кадра во
   всю ширину стыка как границы двух половин листа больше нет.

   ПРИТЕМНЕНИЯ ПОД ЗНАКОМ НЕТ, И ЭТО РЕЗУЛЬТАТ ЗАМЕРА, А НЕ ЗАБЫВЧИВОСТЬ.
   Знак — тёмная плашка со светлыми буквами, и затемнять снимок под ним
   значит сажать его силуэт: угловой градиент rgba(6,8,12,.62) опускал
   контраст плашки против площадки с 3,73 до 2,47 : 1. Притемнение помогает
   светлому знаку на светлом месте; здесь наоборот — площадка под техникой
   светло-серая, и тёмный знак читается по ней сам. */
...
.vc-card{display:flex;flex-direction:column;background:var(--surface);
  border:.3mm solid var(--line);border-radius:6mm;padding:3.5mm}
...
.vc-card-l{margin-top:3mm;padding-top:2.5mm;border-top:.3mm solid var(--line);
 ...
 */
function photoCard() {
  /* Кадр вырос до 198 мм по ширине — на печати это 254 dpi у файла 1920 px
     и 282 у 2200. Берём широкий: он идёт только на эту страницу, лендинга и
     каталога не касается вовсе. */
  ...
  /* ВЫСОТА КАДРА — ПРИМЕРНО ТРЕТЬ ЛИСТА, и взята она не из воздуха, а из
     низа: поля карточек и выноски поджаты, промежутки между нижними блоками
     тоже. Считается от нижней кромки кадра, а не от его высоты: всё, что
     ниже, отмеряется от этой отметки.
     С QR-кодом плашка контактов выше на 18 мм, и такой высоты на листе нет:
     кадр отдаёт её признаком, а не правкой руками, когда впишут домен. */
  const SEAM = QR ? 84 : 102;   // нижняя кромка кадра, мм от верха листа
  const MARK = 11;    // отступ логотипа от нижней и левой кромки кадра, мм
  return `<div class="vc">`
    ...
    /* Знак больше не выходит за кромку кадра, и верхнее поле текста ему
       ничего не должно: остаётся обычный воздух. */
    ...
    + `<div style="margin-top:auto;padding-top:3.5mm">${legal()}</div>`
    + `</div></div>`;
}
